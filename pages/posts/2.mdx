export const meta = {
  title: 'Howto: Make Your Banner Parallax',
...
many differences and would seem easier to implement. And that would be right, **_if_** we were going for the janky slideshow asthetic.
The poor javascript thread running on your browser would try it's best to keep up but left in the dust when you flick the screen away on your mobile device.
...
recommend reading it and point out one thing that was not true in my experience, namely, that mobile Safari would flatten parallax unless you add `position: sticky` ;
in addition to reversing the effect (yikes). Thankfully, this did not happen on my mobile ios device.
...
wasn't my first choice. In fact, some browsers [don't take into account perspective](https://developer.chrome.com/blog/performant-parallaxing/#assorted-bugs-and-workarounds)
...
<figure style={{width: '50%'}}>
  <Image width={300} height={600} quality={95} src={Recording} alt='gif of banner moving' />
  Hey, pretty neat.
</figure>
...
Just setting `left: -1px` worked to fix a lot of the issues I was having in Chrome. Just messed with it, move things slightly back, up/down.
